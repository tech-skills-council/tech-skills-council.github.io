...
(function () {
  ...
  function problems(data) {
    ...
    REQUIRED[kind].forEach(function (n) {
      var v = data[n];
      if (!v || String(v).length < 2) bad.push(n);
    });
  ...
  form.addEventListener('submit', function (ev) {
    ...
    if (bad.length) {
      markBad(bad);
      say('bad', bad.indexOf('why_join') !== -1 || bad.indexOf('relevant_experience') !== -1
        ? 'A couple of the written answers are too short — give us a few sentences to work with.'
        : 'Some fields need attention — check the highlighted rows.');
      return;
    }
